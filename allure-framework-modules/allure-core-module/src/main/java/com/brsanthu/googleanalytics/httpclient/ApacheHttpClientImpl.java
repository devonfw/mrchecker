...
public class ApacheHttpClientImpl implements HttpClient {
	...
	protected List<NameValuePair> createNameValuePairs(HttpRequest req) {
		...
		req.getBodyParams()
				.forEach((key, value) -> parmas.add(new BasicNameValuePair(key, value)));
		return parmas;
	}
	...
	public HttpResponse post(HttpRequest req) {
		...
		try {
			...
			resp.setStatusCode(httpResp.getStatusLine()
					.getStatusCode());
	...
	public HttpBatchResponse postBatch(HttpBatchRequest req) {
		...
		try {
			List<List<NameValuePair>> listOfReqPairs = req.getRequests()
					.stream()
					.map(this::createNameValuePairs)
