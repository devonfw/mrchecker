 ...
 * Analytics (GA). Clients needs to instantiate this object with {@link GoogleAnalyticsConfig} and
 * {@link DefaultRequest}. Configuration contains sensible defaults so one could just initialize using one of the
 * convenience constructors. This object is ThreadSafe and it is intended that clients create one instance of this for
 * each GA Tracker Id and reuse each time an event needs to be posted. This object contains resources which needs to be
 * shutdown/disposed. So {@link #close()} method is called to release all resources. Once close method is called, this
 * instance cannot be reused so create new instance if required.
 */
public class GoogleAnalyticsImpl implements GoogleAnalytics, GoogleAnalyticsExecutor {
		...
		if (isSubmitBatch(force)) {
			...
			synchronized (currentBatch) {
				...
				if (isSubmitBatch(force)) {
					...
					httpClient.postBatch(new HttpBatchRequest().setUrl(config.getBatchUrl())
							.setRequests(currentBatch));
					currentBatch.clear();
				}
			}
		}
	...
	protected void processCustomDimensionParameters(GoogleAnalyticsRequest<?> request, HttpRequest req) {
		...
		for (String defaultCustomDimKey : defaultRequest.customDimensions()
				.keySet()) {
			customDimParms.put(defaultCustomDimKey, defaultRequest.customDimensions()
					.get(defaultCustomDimKey));
		}
	...
	protected void processCustomMetricParameters(GoogleAnalyticsRequest<?> request, HttpRequest req) {
		...
		for (String defaultCustomMetricKey : defaultRequest.custommMetrics()
				.keySet()) {
			customMetricParms.put(defaultCustomMetricKey, defaultRequest.custommMetrics()
					.get(defaultCustomMetricKey));
		}
	...
	public PageViewHit pageView(String url, String title) {
		return pageView().documentUrl(url)
				.documentTitle(title);
	}
	...
	public ScreenViewHit screenView(String appName, String screenName) {
		return screenView().applicationName(appName)
	...
	public SocialHit social(String socialNetwork, String socialAction, String socialTarget) {
		return social().socialNetwork(socialNetwork)
