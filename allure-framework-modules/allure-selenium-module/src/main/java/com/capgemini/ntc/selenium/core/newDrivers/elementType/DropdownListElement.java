...
public class DropdownListElement extends BasicElement implements IBasicElement {
	...
	public void selectDropdownByIndex(int index) {
		...
		List<WebElement> list = this.getObject().getOptions();

		if (!list.get(index).isSelected()) {
			throw new BFComponentStateException( ElementType.DROPDOWN.toString(), "select",
					"Option with index: " + String.valueOf(index) + " should be set from in "
	...
	public void selectDropdownByValue(String value) {
		...
		if (!this.isDropdownElementSelectedByValue(value)) {
            throw new BFComponentStateException(ElementType.DROPDOWN.toString(), "select",
                    "Option with value: " + String.valueOf(value) + " should be set from in "
                            + this.getObject().toString());
        }
	}
	...
	public void selectDropdownByVisibleText(String value) {
		...
		if (!flag) {
			throw new RuntimeException(
					"Option with text: " + value + " wasn't selected in " + this.getObject().toString());
		}
	}

	public List<String> getAllSelectedOptionsText() {
		List<WebElement> list = this.getObject().getAllSelectedOptions();
		return this.getValuesFromWebElements(list);
	}

	public String getFirstSelectedOptionText() {
		String output = this.getObject().getFirstSelectedOption().getText().trim();
		return output;
	}
	...
	private List<String> getValuesFromWebElements(List<WebElement> list) {
		...
		for (int i = 0; i < list.size(); i++) {
			output.add(list.get(i).getText().trim());
		}
		return output;
	}
}
