...
public class NavigationBarElement extends BasicElement {
    ...
    public List<String> getItemsTextList(){
        ...
        for(int i = 0; i < listElems.size(); i++){
            out.add(listElems.get(i).getText());
        }
        return out;
    }
    ...
    public String getActiveItemText(){
        ...
        for(int i = 0; i < listItems.size(); i++){
            if (listItems.get(i).getAttribute("class").contains("active")){
                return listItems.get(i).getText();
            }
        }
    ...
    public void clickItemByIndex(int index){
        if( index > 0 && index >= this.getItems().size() ){
            throw new BFInputDataException("Index " + String.valueOf(index) + " larger than list's size: " + String.valueOf(this.getItems().size()));
        }
    ...
    public void clickItemByText(String text){
        for (int i = 0; i < this.getItems().size(); i++){
            if (this.getItems().get(i).getText().equals(text)){
