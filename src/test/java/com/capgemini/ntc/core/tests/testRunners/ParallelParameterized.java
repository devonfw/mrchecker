package com.capgemini.ntc.core.tests.testRunners;
...
import com.capgemini.ntc.core.tests.testRunners.core.ParametrizedTestMethodsFilter;
 ...
 * <code>&#064;RunWith(ParallelParameterized.class)</code> and place <code>&#064;Parameters</code> annotation on each
 * test method which requires parameters. Nothing more needed - no special structure, no dirty tricks.
 * </p>
 * <br>
 * <h2>Contents</h2> <b> <a href="#p1">1. Parameterising tests</a><br>
 * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="#a">a. Parameterising tests via values in annotation</a><br>
 * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="#b">b. Parameterising tests via a method that returns parameter values</a><br>
 * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="#c">c. Parameterising tests via external classes</a><br>
 ...
 * an array of Strings, where each string contains the argument values separated by a comma or a pipe "|".
 *
 * <pre>
 *   &#064;Test
 ...
 * </pre>
 *
 * Sometimes you may be interested in passing enum values as parameters, then you can just write them as Strings like
 * this:
 *
 * <pre>
 * &#064;Test
 ...
 * </pre>
 *
 * <h4 id="b">b. Parameterising tests via a method that returns parameter values</h4>
 * <p>
 * Obviously passing parameters as strings is handy only for trivial situations, that's why for normal cases you have a
 * method that gives you a collection of parameters:
 *
 * <pre>
 *   &#064;Test
 ...
 * </pre>
 * <p>
 * The <code>method</code> argument of a <code>@Parameters</code> annotation can be ommited if the method that provides
 * parameters has a the same name as the test, but prefixed by <code>parametersFor</code>. So our example would look
 * like this:
 *
 * <pre>
 *   &#064;Test
 ...
 * </pre>
 *
 * If we had more than just two Person's to make, we would get redundant, so JUnitParams gives you a simplified way of
 * creating objects to be passed as params. You can omit the creation of the objects and just return their constructor
 ...
 * provide parameters to a single test method. You can easily do that like this:
 *
 * <pre>
 *   &#064;Test
 ...
 * </pre>
 *
 * But if you want to process the data from the CSV file a bit to use it in the test method arguments, you need to use
 * an <code>IdentityMapper</code>. Look:
 *
 * <pre>
 *   &#064;Test
 ...
 * value in the test. This is where the converters become handy. It's enough to annotate a parameter with a
 * <code>&#064;ConvertParam</code> annotation, give it a converter class and possibly some options (like date format in
 * this case) and you're done. Here's an example:
 * 
 * <pre>
 * &#064;Test
 ...
 * <p>
 * You can easily use JUnitParams together with Spring. The only problem is that Spring's test framework is based on
 * JUnit runners, and JUnit allows only one runner to be run at once. Which would normally mean that you could use only
 * one of Spring or JUnitParams. Luckily we can cheat Spring a little by adding this to your test class:
 *
 * <pre>
 ...
 * <p>
 * Tests show up in your IDE as a tree with test class name being the root, test methods being nodes, and parameter sets
 * being the leaves. If you want to customize the way an parameter object is shown, create a <b>toString</b> method for
 * it.
 * <h4>Empty parameter sets</h4>
 * <p>
 * If you create a parameterised test, but won't give it any parameter sets, it will be ignored and you'll be warned
 * about it.
 * <h4>Parameterised test with no parameters</h4>
 * <p>
 * If for some reason you want to have a normal non-parameterised method to be annotated with @Parameters, then fine,
 * you can do it. But it will be ignored then, since there won't be any params for it, and parameterised tests need
 * parameters to execute properly (parameters are a part of test setup, right?)
 * <h4>JUnit Rules</h4>
 * <p>
 * The runner for parameterised test is trying to keep all the @Rule's running, but if something doesn't work - let me
 * know. It's pretty tricky, since the rules in JUnit are chained, but the chain is kind of... unstructured, so
 * sometimes I need to guess how to call the next element in chain. If you have your own rule, make sure it has a field
 * of type Statement which is the next statement in chain to call.
