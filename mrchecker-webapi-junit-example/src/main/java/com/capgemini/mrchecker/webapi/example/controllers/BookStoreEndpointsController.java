...
public class BookStoreEndpointsController extends BasePageWebAPI {
	@Override
	...
	public AllBooksModel getAllBooks() {
		AllBooksModel books = PerformApiAction.getFromHost(DEMO_QA_API_URL.getValue(), Endpoints.BOOKSTORE_GET_ALL_BOOKS_ENDPOINT.getEndpoint())
	...
	public BookModel getBookByISBN(String isbn) {
		BookModel bookDetails = PerformApiAction.getFromHost(DEMO_QA_API_URL.getValue(), Endpoints.BOOKSTORE_GET_BOOK_DETAILS_ENDPOINT.getEndpoint(isbn))
				.as(BookModel.class);
	...
	public AddedListOfBooksModel postAddBookToUser(String userID, String isbn) {
		...
		AddedListOfBooksModel response = PerformApiAction.postToHost(DEMO_QA_API_URL.getValue(), Endpoints.BOOKSTORE_ADD_LIST_OF_BOOKS_ENDPOINT.getEndpoint(), booksList, 201)
	...
	public void deleteBooksFromUserCollection(String userID) {
		ExtractableResponse response = PerformApiAction.deleteFromHost(DEMO_QA_API_URL.getValue(), Endpoints.BOOKSTORE_REMOVE_LIST_OF_BOOKS_FOR_USER_ENDPOINT.getEndpoint(userID), 204);
		assertNotNull(response, "No response was returned");
	}
}
