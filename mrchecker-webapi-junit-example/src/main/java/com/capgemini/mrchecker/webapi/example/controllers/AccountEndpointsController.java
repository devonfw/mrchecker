...
public class AccountEndpointsController extends BaseEndpointWebAPI {
	...
	public UserModel loginUser(User user) {
		...
		UserModel userDetails = postToLoginHost(DEMO_QA_API_URL.getValue(), Endpoints.ACCOUNT_LOGIN_ENDPOINT.getEndpoint(), userRegistration)
				.as(UserModel.class);
	...
	public GetUserResultModel getUserDetails(String userID) {
		GetUserResultModel userModel = getFromHostLoggedUser(DEMO_QA_API_URL.getValue(), Endpoints.ACCOUNT_GET_USER_ENDPOINT.getEndpoint(userID))
