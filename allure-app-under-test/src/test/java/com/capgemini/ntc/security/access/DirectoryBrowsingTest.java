package com.capgemini.ntc.security.access;
...
/**
 * The test verifies that directory browsing is disabled. OWASP ASVS requirement V4.5: Verify that directory browsing is
 * disabled unless deliberately desired. Additionally, applications should not allow discovery or disclosure of file or
 * directory metadata, such as Thumbs.db, .DS_Store, .git or .svn folders. Purpose: You don't want the attacker to learn
 * about the system more then needed and he certainly does not need to list files in the directories on your server
 * (unless you deliberately desire this feature). Putting private content of your e.g. .git folder into your
 * distribution package to be uploaded to the server and downloaded by the attacker because of directory browsing
 * feature is a mortal sin on its own.
 *
