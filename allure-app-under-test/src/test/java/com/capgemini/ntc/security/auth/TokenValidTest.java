...
/**
 * The test verifies, that the application can not be accessed via a fabricated token. OWASP ASVS requirement V3.10:
 * Verify that only session ids generated by the application framework are recognized as active by the application.
 * Purpose: If one manages to access the application via a fabricated token, then it is game over. Read also: [1]
