...
**NOTE:** methods load() and isLoaded() are called right after page object initialization. Keep that in mind when implementing test class.
...
*Note*: The examples displayed in the _cssSelector.docx_ file use the Selenium method `driver.findElement()` to find elements. However, using this framework we recommend  `findElementDynamic()` or `findElementQuietly().findElementDynamic()` allows waiting for dynamic elements, for example buttons that pop up.
