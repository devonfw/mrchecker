...
public class CheckBox extends BasicElement {
	...
	private List<WebElement> getCheckBoxesList(){
		return this.getElement().findElements(inputChildsSelector);
	}
	...
	private void setCheckBoxByIndexTo(int index, boolean destination){
		...
		boolean currentState = this.getCheckBoxesList().get(index).isSelected();
	...
	private int getCheckBoxIndexByText(String text){
		...
		for (int i = 0; i < textsList.size(); i++){
			if (textsList.get(i).equals(text.trim())){
				return i;
			}
		}
	...
	private int getCheckBoxIndexByValue(String value){
		...
		for (int i = 0; i < checkBoxesList.size(); i++){
			if (checkBoxesList.get(i).getAttribute("value").equals(value.trim())){
				return i;
			}
		}
	...
	private void setCheckBoxToByAttribute(String value, String attribute, boolean destination){
		...
		for(int i = 0; i < checkboxesList.size(); i++){
			...
			if (currentElement.getAttribute(attribute).equals(value) && currentElement.isSelected() != destination){
	...
	private void setAllCheckBoxesTo(boolean destination){
		List<WebElement> checkboxesList = this.getCheckBoxesList();
		for (int i = 0; i < checkboxesList.size(); i++){
			if (checkboxesList.get(i).isSelected() != destination){
				checkboxesList.get(i).click();
			}
		}

		if (!this.isAllCheckBoxesSetTo(destination)){
			throw new BFComponentStateException(ElementType.CHECKBOX.toString(), "set/unset", "setting to " + String.valueOf(destination));
		}
	}
