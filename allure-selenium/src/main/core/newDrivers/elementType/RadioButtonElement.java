...
public class RadioButtonElement extends BasicElement implements IBasicElement {
	...
	private void setItems() {
		setListItems( this.getElement().findElements(inputChildsSelector));
	}
	...
	private WebElement getItemByText(String visibleText) {
		for (int i = 0; i < this.getItemsCount(); i++){
			if (this.listElements.get(i).getText().equals(visibleText)){
				return this.listElements.get(i);
			}
		}
		throw new BFElementNotFoundException(getElementTypeName() + " with text: " + visibleText + " wasn't found in " + this.getTextArray().toString());
	}

	private WebElement getItemByValue(String value) {
		this.setItems();
		for (int i = 0; i < this.getItemsCount(); i++){
			if (this.listElements.get(i).getAttribute("value").equals(value)){
				return this.listElements.get(i);
			}
		}
		throw new BFElementNotFoundException( getElementTypeName() + " with value: " + value + " wasn't found in " + this.getTextArray().toString());
	}
	...
	private WebElement getSelectedItem(){
		...
		for (int i = 0; i < this.listElements.size(); i++) {
			if (this.isItemSelected(this.listElements.get(i)) || isClassContainSelectionAttributes(this.listElements.get(i).getAttribute("class"))) {
				return this.listElements.get(i);
			}
		}
		throw new BFElementNotFoundException("Any element is selected in " + this.getTextArray().toString());
	}
}