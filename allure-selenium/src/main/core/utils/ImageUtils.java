package com.example.selenium.core.utils;
...
public class ImageUtils {
	...
	private static BufferedImage getWebElementScreenshot(WebElement we) throws IOException {
		...
		BufferedImage webElementScreenshot = fullImg.getSubimage(we.getLocation().getX(), we.getLocation().getY() + 1,
				we.getSize().getWidth(), we.getSize().getHeight());
		BufferedImage webElementScreenshotWithAlfa = new BufferedImage(
				we.getSize().getWidth(),
				we.getSize().getHeight(),
		...
		FileUtils.copyFile(screenshotFile, new File(System
	...
	private static String buildScrollScript(Point point) {
		return new StringBuilder("window.scrollTo(")
				.append(point.x)
				.append(",")
				.append(point.y)
				.append(");")
